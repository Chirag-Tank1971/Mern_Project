  ...
  const addAgent = async (e) => {
    ...
    try {
      await axios.post(
        ...
        { ...form, mobile: fullMobileNumber }, // Send combined mobile number
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
      );
            ...
            >
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-gray-400">{agent.mobile}</p>
                </div>
...
const Tasks = ({ agentId }) => {
  ...
  const fetchTasks = async () => {
    ...
    try {
      const response = await axios.get(`http://localhost:5000/api/tasks/${agentId}`, {
        headers: {
